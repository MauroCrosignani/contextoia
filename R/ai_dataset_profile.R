...
#' Resumir un dataset para darle contexto seguro a una IA
#'
#' Genera un resumen programatico y prudente de un `data.frame` o tibble,
#' evitando exponer ejemplos reales de texto libre, identificadores o etiquetas
#' de entidad cuando eso podria aumentar el riesgo de divulgacion.
#'
#' @param data `data.frame` o tibble a resumir.
#' @param nombre_dataset Nombre opcional para mostrar en la salida.
#' @param config Lista declarativa opcional con ajustes por columna.
#' @param tipo_fuente Tipo de fuente declarado por el usuario, por ejemplo
#'   `"gca"`, `"gca2"` u `"odbc"`.
#' @param archivo_fuente Ruta opcional al archivo fuente usado para inferir
#'   contexto GCA/GCA2.
#' @param metadata_dir Carpeta opcional con metadata externa en JSON.
#' @param modo `"normal"` o `"conservador"`.
#' @param salida `"texto"` para salida pegable en una conversacion o
#'   `"estructura"` para devolver la lista estructurada.
#'
#' @return Un texto si `salida = "texto"` o una lista si
#'   `salida = "estructura"`.
#' @export
resumen_de <- function(
  data,
